...
def _render_validation(validation_view_model: dict[str, Any], *, nav_prefix: str = '', available_pages: set[str] | None = None) -> str:
    ...
    body = (
        ...
        "<span id='replay-attention-visible-count'></span></div>"
        ...
        "link.href=url;"
        "link.download='replay_attention_watchlist.csv';"
        ...
        "const matches=(!level||rowLevel.indexOf(level)!==-1)&&(!owner||rowOwner.indexOf(owner)!==-1)&&(!reason||rowReason.indexOf(reason)!==-1);"
        ...
        "if(preset==='high-only'){if(levelInput){levelInput.value='high';}if(ownerInput){ownerInput.value='';}if(reasonInput){reasonInput.value='';}}"
        "else if(preset==='governance-only'){if(levelInput){levelInput.value='';}if(ownerInput){ownerInput.value='validation governance';}if(reasonInput){reasonInput.value='';}}"
        "else if(preset==='domain-gap-only'){if(levelInput){levelInput.value='';}if(ownerInput){ownerInput.value='';}if(reasonInput){reasonInput.value='domain_coverage_gap';}}"
        "else {if(levelInput){levelInput.value='';}if(ownerInput){ownerInput.value='';}if(reasonInput){reasonInput.value='';}}"
        "applyReplayAttentionFilters();"
        "});}"
        "if(resetButton){resetButton.addEventListener('click',function(){if(levelInput){levelInput.value='';}if(ownerInput){ownerInput.value='';}if(reasonInput){reasonInput.value='';}if(sortSelect){sortSelect.value='default';}if(presetSelect){presetSelect.value='none';}applyReplayAttentionFilters();});}"
