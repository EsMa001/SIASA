...
def _render_comparison(country_profile_read_models: dict[str, dict[str, Any]], *, nav_prefix: str = '', available_pages: set[str] | None = None) -> str:
    ...
    body = (
        ...
        f"<option value=''>— select —</option>{''.join(f'<option value={chr(39)}{html.escape(cid)}{chr(39)}>{html.escape(cid)}</option>' for cid in sorted(country_profile_read_models))}"
        "</select></div>"
        ...
        "var mwData={};"
        + "".join(
            f"mwData['{html.escape(cid)}']={{status:'{html.escape(str(profile.get('multi_domain_status','n/a')))}',domains:'{html.escape(', '.join(f'{d}:{s}' for d,s in sorted(profile.get('domain_states',{}).items())))}',coverage:'{html.escape(str(profile.get('coverage','n/a')))}',confidence:'{html.escape(str(profile.get('confidence','n/a')))}',drivers:'{html.escape(', '.join(str(d) for d in profile.get('drivers',[])))}'}};"
            for cid, profile in sorted(country_profile_read_models.items())
        )
        ...
        "+'<div style=\"font-size:12px;color:#dae2fd;\"><b>Status:</b> '+d.status+'<br><b>Domains:</b> '+d.domains+'<br><b>Coverage:</b> '+d.coverage+'<br><b>Confidence:</b> '+d.confidence+'<br><b>Drivers:</b> '+d.drivers+'</div>';}"
