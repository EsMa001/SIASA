...
def _render_validation(validation_view_model: dict[str, Any], *, nav_prefix: str = '', available_pages: set[str] | None = None) -> str:
    ...
    body = (
        ...
        "function renderReplayAttentionActiveState(level,owner,reason,text,sortKey,preset){if(!activeStateEl){return;}const parts=[];if(preset&&preset!=='none'){parts.push('preset='+preset);}if(level){parts.push('level='+level);}if(owner){parts.push('owner='+owner);}if(reason){parts.push('reason='+reason);}if(text){parts.push('search='+text);}if(sortKey&&sortKey!=='default'){parts.push('sort='+sortKey);}activeStateEl.textContent=parts.length?('Active: '+parts.join(' | ')):'Active: default';}"
        ...
        "renderReplayAttentionActiveState(level,owner,reason,text,sortKey,preset);"
