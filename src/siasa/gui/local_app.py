...
def _render_validation(validation_view_model: dict[str, Any], *, nav_prefix: str = '', available_pages: set[str] | None = None) -> str:
    ...
    body = (
        ...
        "}"
        "if(levelInput){levelInput.addEventListener('input',applyReplayAttentionFilters);}"
        "if(ownerInput){ownerInput.addEventListener('input',applyReplayAttentionFilters);}"
        "if(reasonInput){reasonInput.addEventListener('input',applyReplayAttentionFilters);}"
        "if(sortSelect){sortSelect.addEventListener('change',applyReplayAttentionFilters);}"
        "if(resetButton){resetButton.addEventListener('click',function(){if(levelInput){levelInput.value='';}if(ownerInput){ownerInput.value='';}if(reasonInput){reasonInput.value='';}if(sortSelect){sortSelect.value='default';}applyReplayAttentionFilters();});}"
