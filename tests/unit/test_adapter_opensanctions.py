"""Tests for OpenSanctions adapter (SwR-082, AP-15.7).

TC-SwR-082-001: Verify OpenSanctionsAdapter fetches entity counts and new
listings from the OpenSanctions API, normalizes to NormalizedRecord dicts.
"""
from __future__ import annotations
from datetime import datetime, timezone
import pytest
from siasa.adapters.opensanctions import OpenSanctionsAdapter
from siasa.adapters.base import FetchResult

SAMPLE_RESPONSE = {
    "total": {"value": 350},
    "results": [
        {"id": "Q123", "caption": "Entity A", "schema": "Person"},
        {"id": "Q456", "caption": "Entity B", "schema": "Company"},
        {"id": "Q789", "caption": "Entity C", "schema": "Person"},
    ],
}
EMPTY_RESPONSE = {"total": {"value": 0}, "results": []}
FIXED_NOW = datetime(2026, 6, 24, 12, 0, 0, tzinfo=timezone.utc)

def _make_adapter(fetch_fn=None, country_ids=None, now_fn=None, max_retries=3):
    if country_ids is None:
        country_ids = ("IRN", "RUS")
    if now_fn is None:
        now_fn = lambda: FIXED_NOW
    return OpenSanctionsAdapter(
        country_ids=country_ids,
        fetch_json=fetch_fn or (lambda url: SAMPLE_RESPONSE),
        now_provider=now_fn,
        retry_sleep=lambda _: None,
        max_retries=max_retries,
    )

class TestBasicFetch:
    def test_returns_fetch_result(self):
        result = _make_adapter().fetch()
        assert isinstance(result, FetchResult)
        assert result.is_success is True

    def test_records_have_required_fields(self):
        result = _make_adapter().fetch()
        assert len(result.records) > 0
        for rec in result.records:
            assert {"country_id", "period", "signal_key", "value", "quality_flag"}.issubset(rec.keys())

    def test_signal_keys_contain_opensanctions_prefix(self):
        for rec in _make_adapter().fetch().records:
            assert rec["signal_key"].startswith("opensanctions_")

    def test_values_are_numeric(self):
        for rec in _make_adapter().fetch().records:
            assert isinstance(rec["value"], (int, float))

class TestSanctionsParsing:
    def test_entity_count_extracted(self):
        result = _make_adapter(country_ids=("IRN",)).fetch()
        counts = [r for r in result.records if r["signal_key"] == "opensanctions_entity_count"]
        assert len(counts) == 1
        assert counts[0]["value"] == 350.0

    def test_new_listings_extracted(self):
        result = _make_adapter(country_ids=("IRN",)).fetch()
        listings = [r for r in result.records if r["signal_key"] == "opensanctions_new_listings"]
        assert len(listings) == 1
        assert listings[0]["value"] == 3.0  # 3 results

    def test_two_signals_per_country(self):
        result = _make_adapter(country_ids=("IRN",)).fetch()
        assert len(result.records) == 2

    def test_multiple_countries(self):
        result = _make_adapter(country_ids=("IRN", "RUS")).fetch()
        assert len(result.records) == 4  # 2 signals x 2 countries

class TestCountryMapping:
    def test_only_configured_countries_returned(self):
        result = _make_adapter(country_ids=("IRN",)).fetch()
        assert all(r["country_id"] == "IRN" for r in result.records)

    def test_empty_country_ids_returns_empty(self):
        result = _make_adapter(country_ids=()).fetch()
        assert result.is_success is True
        assert result.records == []

class TestPeriodHandling:
    def test_period_is_iso_date(self):
        for rec in _make_adapter().fetch().records:
            datetime.strptime(rec["period"], "%Y-%m-%d")

class TestErrorHandling:
    def test_api_error_returns_unsuccessful(self):
        def fail(url): raise ConnectionError("timeout")
        result = _make_adapter(fetch_fn=fail, max_retries=0).fetch()
        assert result.is_success is False

    def test_malformed_response_handled(self):
        result = _make_adapter(fetch_fn=lambda url: {"broken": True}).fetch()
        assert isinstance(result, FetchResult)

    def test_partial_country_failure(self):
        first_call = [True]
        def partial(url):
            if first_call[0]:
                first_call[0] = False
                raise ConnectionError("fail")
            return SAMPLE_RESPONSE
        result = _make_adapter(fetch_fn=partial, max_retries=0, country_ids=("IRN", "RUS")).fetch()
        # IRN fails, RUS succeeds
        assert result.is_success is True
        assert any(r["country_id"] == "RUS" for r in result.records)

class TestRetryLogic:
    def test_retries_on_failure(self):
        calls = [0]
        def flaky(url):
            calls[0] += 1
            if calls[0] < 3: raise ConnectionError("transient")
            return SAMPLE_RESPONSE
        result = _make_adapter(fetch_fn=flaky, max_retries=3).fetch()
        assert result.is_success is True

    def test_retry_sleep_called(self):
        sleeps = []
        def fail(url): raise ConnectionError("fail")
        a = _make_adapter(fetch_fn=fail, max_retries=1)
        a.retry_sleep = lambda d: sleeps.append(d)
        a.fetch()
        assert len(sleeps) >= 1

class TestSourceMetadata:
    def test_source_id(self):
        assert _make_adapter().source_id == "SRC-OPENSANCTIONS"
    def test_domain_is_B(self):
        assert _make_adapter().domain == "B"
    def test_quality_flag(self):
        for rec in _make_adapter().fetch().records:
            assert rec["quality_flag"] == "opensanctions_api"

class TestDiagnostics:
    def test_success_diagnostics(self):
        assert "opensanctions" in _make_adapter().fetch().diagnostics.lower()
    def test_empty_diagnostics(self):
        d = _make_adapter(country_ids=()).fetch().diagnostics
        assert "no" in d.lower() or "0" in d

class TestFreshnessMetadata:
    def test_freshness_hours(self):
        for rec in _make_adapter().fetch().records:
            assert isinstance(rec["freshness_hours"], (int, float))
    def test_expected_source_count(self):
        for rec in _make_adapter().fetch().records:
            assert rec["expected_source_count"] == 1

class TestInjectableFetch:
    def test_custom_fetch_used(self):
        calls = []
        def track(url): calls.append(url); return SAMPLE_RESPONSE
        _make_adapter(fetch_fn=track).fetch()
        assert len(calls) >= 1
    def test_url_contains_country(self):
        calls = []
        def track(url): calls.append(url); return SAMPLE_RESPONSE
        _make_adapter(fetch_fn=track, country_ids=("IRN",)).fetch()
        assert any("irn" in url for url in calls)
