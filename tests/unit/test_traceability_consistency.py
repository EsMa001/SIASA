...
def test_build_repo_closure_report_aggregates_all_governed_slices() -> None:
    ...
    assert report["slice_ids"] == [
        ...
        "ap-30-historischer-backfill",
        "backfill-pit-monotonicity",
    ...
    assert report["slices"][2]["slice_id"] == "ap-30-historischer-backfill"
    assert report["slices"][2]["summary"] == {"closed": 4, "at_risk": 0}
    assert report["slices"][3]["slice_id"] == "backfill-pit-monotonicity"
    assert report["slices"][3]["summary"] == {"closed": 1, "at_risk": 0}
    assert report["slices"][4]["slice_id"] == "baseline-and-status-engines"
    assert report["slices"][4]["summary"] == {"closed": 8, "at_risk": 0}
    ...
    assert report["slices"][10]["summary"] == {"closed": 5, "at_risk": 0}
    ...
    assert report["slices"][15]["summary"] == {"closed": 3, "at_risk": 0}
    assert report["slices"][16]["slice_id"] == "ucdp-server-side-query"
    assert report["slices"][16]["summary"] == {"closed": 1, "at_risk": 0}
    assert report["slices"][17]["slice_id"] == "validation-and-backtest"
    assert report["slices"][17]["summary"] == {"closed": 13, "at_risk": 0}
