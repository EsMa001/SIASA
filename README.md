...
- der governed Live-Runtime-Pfad erzeugt jetzt zusätzlich `readmodels/validation_backtest.json` als transparenten `runtime_support_check`; diese Sicht ist explizit keine historische Referenzfall-Backtest-Wertung, enthält jetzt aber eine fallübergreifende Portfolio-Summary über alle im Run validierbaren Länder sowie eine kuratierte Reference-Case-Library aus dem Repo als sichtbaren Tiefenanker für spätere historische Validierung
