...
1. Validation realism depth / analyst handoff refinement (default internal track)
2. G2 – External credentialed-source activation follow-through (conditional trigger only)
3. G5 – Optional server-backed multi-user governance (deferred by default)
...
- the default internal next package family is now validation realism depth / analyst handoff refinement on top of the closed replay-attention baseline (`VAL-WP-001..012`)
...
### First: Validation realism depth / analyst handoff refinement
Because this is the current highest-value internal track without requiring any new external unlock:
