...
Phase 0 + **Phase 1 vollständig** (AP-16/26/27/28) sind abgeschlossen (s. §0). **Nächster Schritt: Phase 2 (echte Daten)** — **AP-29** (reales Event-Set + S0-Negative + Onset-Daten; **Owner-Kuratierung nötig**) → **AP-30** (Backfill GDELT/GDACS/WB + Point-in-Time, größter Block) → AP-31 → AP-32. Erst damit werden die AP-28-Skill-Zahlen **und** die an AP-30 gebundenen AP-24-Schwellen (s. §⭐) aussagekräftig. Alternativ offene Audit-Restpunkte aus §4 (3 Verifikations-Routen SyR-051/052/053; Cluster-B-Lokalgate) oder die offenen AP-24-Owner-Policy-Entscheidungen (§⭐). **Owner-Vorbedingung für AP-29:** den echten S0-Negativ-/Onset-Labelsatz kuratieren (AP-27 lieferte nur Mechanik + 1 Beispiel).
