...
- AP-19 is now closed: release-failure-drill output now includes a deterministic trend baseline (`operator_failure_drill_trend_baseline`) so recurring no-go clusters can be tracked over follow-up snapshots; next serial planning focus is AP-20 remediation-closure governance (actionability + unresolved-age guardrails)
