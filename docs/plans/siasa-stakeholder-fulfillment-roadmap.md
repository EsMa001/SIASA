...
- VAL-WP-014 readiness analyst-briefing contextual deep links: the readiness-facing `Analyst Briefing — What matters now?` is now a context-carrying action surface instead of static page-name text; prioritized `country_gap`, `stale_priority`, `validation_attention`, `traceability_risk`, `operability_cluster`, and `stale_remediation_action_plan` items now carry deterministic `target_href`, while validation-attention rows additionally expose `Create Annotation Draft` as a direct action link into `annotations.html` with the full replay-attention prefill context preserved; `_render_readiness(...)` now renders safe-gated `analyst-briefing-target-link` / `analyst-briefing-action-link` anchors in the primary-focus summary and the item table; targeted RED/GREEN tests were added in `tests/unit/test_local_gui.py`, focused GUI regression passed (`23 passed in 108.20s`), full suite `534 passed in 345.76s`, and GUI evidence is visible in `build/local_gui/_val_wp014_briefing_links/readiness.html`; commit `commit pending`
