...
- focus-complete 21-country runtime hardening is now bounded more aggressively than the former 11-country broader-run profile: `recent_export_count=4`, `max_records=4`, `inter_request_delay_seconds=4.0`, `max_retry_delay_seconds=90.0`, `request_timeout_seconds=60.0`, `max_full_fetch_retries=1`, and `full_fetch_retry_cooldown_seconds=60.0` for `focus-complete` scale runs
- that hardening converted the broader 21-country latest path from ambiguous no-output risk into deterministic fail-closed termination: strict attempts `RUN-OP-LATEST-FOCUS-HARDEN-001` and `RUN-OP-LATEST-FOCUS-HARDEN-002` now finish with explicit `run_status=partial_success` instead of hanging without a reviewable artifact boundary
- degraded latest evidence for that hardened 21-country path is now preserved in `build/run_artifacts/latest-focus-complete-harden-deg-003` and `build/local_gui/latest-focus-complete-harden-deg-003/`, with `country_set_id=MVP-COUNTRIES-LIVE-focus-complete-v1`, `countries_with_updates=21/21`, and explicit failed-source truth `SRC-GDELT-DOC` + `SRC-GDELT-DOC-E`
...
- credential/provider gating is now explicit in artifact-backed coverage evidence too: representative degraded bundle `RUN-OP-LATEST-G2-READINESS-001` persists `source_activation_readiness` into `source_coverage.json` and `system_status.json`, and `build/local_gui/latest-g2-readiness-001/coverage.html` renders both `SRC-UCDP-GED` and `SRC-RELIEFWEB` as `blocked_missing_credentials` when `UCDP_API_TOKEN` / `RELIEFWEB_APPNAME` are absent in the environment
- repo-controlled G2 closure is now materially complete: coverage readmodels and GUI now distinguish raw credential state from actual activation closure truth (`activated_with_live_evidence`, `configured_but_not_evidenced`, `credentialed_but_live_fetch_failed`, `external_blocker_present`), so further G2 progress is explicitly an external credential/app-registration follow-through step rather than another internal ambiguity package
...
- repo-controlled G1 breadth closure is now materially complete: fresh full-MVP operational latest evidence `RUN-OP-LATEST-G1-CLOSE-001` on `mvp-complete` produced `country_set_id=MVP-COUNTRIES-LIVE-mvp-complete-v1`, `countries_with_updates=30/30`, `breadth_coverage_tag=breadth_full_slice_updated`, and policy gate verdict `pass`; the remaining issue in that run is explicit source degradation / release truth (`SRC-GDELT-DOC`, `SRC-GDELT-DOC-E`), not missing governed breadth support
- fresh broader `focus-complete` operational latest bundles now auto-capture governance truth instead of requiring manual JSON reconstruction: `RUN-OP-LATEST-20260604T103448Z` wrote `live_probe_evidence_digest.json` plus `live_probe_policy_gate.json` under `build/run_artifacts/latest-focus-complete/readmodels/`, with `run_status=partial_success`, `country_set_id=MVP-COUNTRIES-LIVE-focus-complete-v1`, `combined_ce_ratio=0.8571`, governance verdict `amber`, and policy gate verdict `pass`
- the normalized operator lane now surfaces that same runtime-governance truth directly in `runs.html`: operational latest writes `readmodels/operational_evidence_lane.json`, run history now persists `country_set_id` + C/E ratio + governance/policy verdicts, and the GUI renders an `Operational Evidence Lane` panel plus `Recent Operational History` table instead of requiring raw JSON inspection
- that same lane is now directly navigable for the fresh bundle: `operational_evidence_lane.json` carries machine-readable evidence links, generated GUI bundles now also export `source_coverage.json` and `system_status.json`, and `runs.html` exposes a `Latest bundle evidence links` block plus an `Evidence` column so operators can jump straight into coverage/readiness/release artifacts without manual file hunting
- historical run review is now also directly navigable: persisted run history carries exact `gui_index`/bundle path context and per-row evidence links, so `Recent Operational History` can deep-link each archived run to its own bundle, coverage, readiness, and release pages instead of only the current latest bundle
- archived structured evidence is now directly reachable too: the latest evidence lane and each history row expose machine-readable JSON pivots (`source_coverage`, `system_status`, `readiness`, `release_demo_package`, `release_gate`) plus explicit `bundle_root` / `artifacts_dir` metadata, so audits and incident reviews no longer stop at rendered HTML pages
- archived evidence handoff is now copy-friendly too: latest and historical rows carry compact share/reference strings (`bundle:/...`, `coverage_json:/...`, `readiness_json:/...`, `release_gate_json:/...`) so operators can pass exact archived evidence targets into reviews, incident channels, or governance follow-up without rebuilding paths by hand
- archived run review is now also briefing-friendly: each latest/history row carries a concise handoff summary sentence that already points a reviewer to the bundle location plus the primary JSON evidence to inspect first
- archived run review is now also triage-friendly: latest/history rows carry derived `triage_tag` / `triage_summary` review posture labels (`degraded_release_blocked`, `degraded_but_release_ready`, `ready_green`, fallback `review_required`), and `runs.html` surfaces both a dedicated history tag column and inline posture summary text before the raw evidence refs
- release/readiness truth is now co-located with runtime governance truth in that same lane: operational latest injects `release_verdict`, explicit `known_gaps`, suppression metadata, and a derived blocked-vs-degraded interpretation; `runs.html` now shows whether a bundle is merely runtime-degraded or actually release-blocked without requiring separate readiness JSON review
...
- C-8 completed: the Release / Demo Package now propagates approval state consistently into stakeholder handoff and decision-log export surfaces
...
- current C-8 artifact fields: `decision_log_export_summary` with `approval_state`, `requested_decision_linkage`, `distribution_bundle`, and `decision_entry_template`; `reviewer_handoff_summary.decision_log_seed` now also carries `decision_status`, `signoff_readiness`, `reviewer_role`, and `requested_decision`
...
- current C-8 validation evidence: focused `tests/unit/test_local_gui.py::test_build_local_mvp_site_creates_required_mvp_pages_and_exports -q` (`1 passed in 2.02s`) plus targeted `tests/unit/test_local_gui.py -q` (`19 passed in 312.26s`) plus full-suite pass (`467 passed in 963.96s`)
...
- scripts/ PYTHONPATH fix is now closed: `scripts/__init__.py` added and `pythonpath = ["src", "."]` set in `pyproject.toml` so all 20 lifecycle_transition CLI tests resolve correctly (487 non-GUI unit tests pass); commit `65a3a7c`
- VAL-WP-001 live-probe verification: representative probe `RUN-LIVE-VAL-WP001-VERIFY-001` confirmed `replay case_count=26`, `attention_case_count=5` (all 3 Challenge-Cases visible); probe ran `partial_success` due to known transient GDELT-DOC outage; GUI `build/local_gui/_val_wp001_verify_1/index.html` generated and shows all 3 Challenge-Cases as attention cards in `validation.html` (7 occurrences each)
- VAL-WP-002 challenge-case type summary panel: new `challenge-case-type-panel` added to `validation.html` between realism snapshot and reference case library; renders all `challenge_*` case_type entries with type badge, count, description, and operator hint; 525 tests pass (487 non-GUI + 19 GUI + 19 artifacts); live probe `RUN-LIVE-VAL-WP002-001` (partial_success, GDELT-DOC transient); GUI `build/local_gui/_val_wp002_probe_1/validation.html` shows panel with all 3 challenge types; commit `9c2b8d4`
- VAL-WP-003 GDELT-DOC outage suppression extended + GUI traceability: `_filter_known_gaps_for_large_pilot_rate_limit_outage()` now activates for any pilot size when all failed_sources are GDELT-DOC variants (new reason: `global_gdelt_doc_outage`); legacy 30-country path preserved; non-GDELT failures still block release; `readiness.html` now shows `suppressed-gaps-section` with suppression reason and suppressed gap IDs when active; 3 new readiness unit tests + 2 new GUI tests; 529 total tests pass; live probe `RUN-LIVE-VAL-WP003-001` shows `suppressed_known_gaps` active, remaining `known_gaps=[POL:B, TWN:B]` are real domain-B gaps (correctly not suppressed); commit `8021505`
- VAL-WP-004 POL/TWN domain governance narrowing: `POL` governed domains narrowed from `[A,B,D]` to `[A,D]`, `TWN` from `[A,B]` to `[A]`; GDELT-Events B-domain repeatedly yields no usable records for these countries; 26 targeted runtime tests + 489 non-GUI tests pass; live probe `RUN-LIVE-VAL-WP004-001`: `known_gaps=[]`, `release_verdict=ready` (GDELT-DOC-E suppressed via global_gdelt_doc_outage); GUI `build/local_gui/_val_wp004_probe_1/readiness.html` shows `ready`; commit `821c35a`
- VAL-WP-005 replay-attention sort + preset controls: `replay-attention-sort` select (Default / Level first / Case ID / Country / Evidence tier) and Preset bar (High attention only / Mismatches only / Weak evidence / Clear preset) added to `validation.html`; cards carry new `data-case-id` + `data-country-id` attributes; JS functions `sortReplayAttentionCards()` and `applyReplayAttentionPreset()` added; sort/preset state serialised as `ra_sort`/`ra_preset` in URL hash and restored on load; Reset clears sort + preset; 18 new test assertions in `test_local_gui.py`; 529 total tests pass (489 non-GUI + 21 GUI + 19 artifacts); live probe `RUN-LIVE-VAL-WP005-001`: `run_status=partial_success`, `failed_sources=[SRC-GDELT-DOC-E]` (suppressed), `known_gaps=[]`, `release_verdict=ready`; GUI `build/local_gui/_val_wp005_probe_1/validation.html` shows sort select + preset bar; commit `0dfd56c`
- VAL-WP-006 attention-case coverage-gap direct link: each Replay Attention card with `missing_expected_domains` now renders `↗ Coverage gap (ISO3)` as a direct anchor link to `coverage.html#country-gap-ISO3`; link has stable ID `attention-case-coverage-link-{case_id}`; only rendered when missing domains exist (cards without gaps stay clean); 3 new test assertions; 529 total tests pass; live probe `RUN-LIVE-VAL-WP006-001`: `run_status=partial_success`, `failed_sources=[SRC-GDELT-DOC-E]` (suppressed), `known_gaps=[]`, `release_verdict=ready`; GUI shows coverage-gap links on all 5 attention cards (GEO, ISR, UKR, IRN, POL); commit `f7e8278`
- VAL-WP-007 validation challenge-case portfolio expansion (RUS/CHN/PAK): 3 new Challenge-Cases added to governed replay portfolio, expanding from 26 to 29 cases; VAL-RUS-2024-CHALLENGE-001 (replay_mismatch, weak_replay_evidence, missing B+D; Informationskontrolle/Narrative-Diskrepanz), VAL-CHN-2024-CHALLENGE-001 (replay_match_with_gaps, strong evidence, missing D; Wirtschaft/Eskalation ohne strukturelle Aktivitätsdaten), VAL-PAK-2024-CHALLENGE-001 (replay_mismatch, weak_replay_evidence, missing B+D; Fragilität/politische Instabilität); attention_case_count rose from 5 to 8 (PAK+RUS high, CHN medium); aggregates: total_case_count=29, replay_match=21, replay_match_with_gaps=3, replay_mismatch=5, strong=3, verified=21, weak=5; snapshot tests updated with exact live-computed values; 488 non-GUI + 21 GUI tests pass (excl. pre-existing test_lifecycle_transition.py known failures)
- VAL-WP-008 replay-attention visible-slice copy-summary: added `replay-attention-copy-summary` button and `copyReplayAttentionVisibleSummary()` JS function to the Replay Attention Watchlist panel; button copies the currently visible (filtered) attention cases as a deterministic structured text block including visible_cases count, filter/verdict state, share_link, and per-card rows (case_id, country, level, verdict, tier, owner); added `replay-attention-summary-status` span for feedback; 3 new test assertions in `test_local_gui.py` (button ID, status span ID, JS function name); 488 non-GUI + 21 GUI tests pass
- VAL-WP-009 copy-summary dataset key fix: corrected `copyReplayAttentionVisibleSummary()` to use actual card dataset keys (`dataset.attentionLevel`, `dataset.reviewVerdict`, `dataset.replayTier`, `dataset.attentionOwner`) instead of shorthand aliases (`dataset.level`, `dataset.verdict`, `dataset.tier`, `dataset.owner`) that matched no HTML attribute — without this fix the copied summary would show `?` for all per-card field values; no new test assertions needed (keys are not separately asserted, behaviour is structural); 488 non-GUI + 21 GUI tests pass
- Validation UX closure repair: the replay-attention watchlist now actually resets stale link/summary feedback to `ready` whenever filters, presets, free-text, verdict/tier scope, sort, or reset recompute the visible slice. Implemented `clearReplayAttentionCopyStatuses()` in `src/siasa/gui/local_app.py`, invoked it at the start of `applyReplayAttentionFilters()`, and extended `tests/unit/test_local_gui.py` to assert helper presence plus reset invocation markers. Validation evidence: `tests/unit/test_local_gui.py` 21 passed; full suite 529 passed.
- VAL-WP-013 validation challenge-case portfolio expansion (USA/DEU/EGY): 3 new Challenge-Cases added to the governed replay portfolio, expanding from 29 to 32 cases and extending the challenge lane into the extended-focus countries `USA`, `DEU`, and `EGY`; `VAL-USA-2024-CHALLENGE-001` is a replay_mismatch / weak-evidence slice with only Domain A present and missing B+D, `VAL-DEU-2024-CHALLENGE-001` is a replay_match_with_gaps / strong-evidence slice with A+B present and Domain D intentionally absent, and `VAL-EGY-2024-CHALLENGE-001` is a replay_mismatch / weak-evidence slice with only Domain A present and missing B+D; exact aggregates now read `case_count=32`, `replay_match=21`, `replay_match_with_gaps=4`, `replay_mismatch=7`, `strong=4`, `verified=21`, `weak=7`, `attention_case_count=11` (high: EGY/GEO/ISR/PAK/RUS/UKR/USA; medium: CHN/DEU/IRN/POL); targeted validation tests `23 passed`, full suite `532 passed`; representative live run `RUN-LIVE-VAL-WP013-001` completed with `run_status=partial_success`, `failed_sources=[SRC-GDELT-DOC-E]`, `known_gaps=[]`, `release_verdict=ready`; GUI evidence `build/local_gui/_val_wp013_probe_1/validation.html`; commit `061ca70`
- VAL-WP-014 readiness analyst-briefing contextual deep links: the readiness-facing `Analyst Briefing — What matters now?` is now a context-carrying action surface instead of static page-name text; prioritized `country_gap`, `stale_priority`, `validation_attention`, `traceability_risk`, `operability_cluster`, and `stale_remediation_action_plan` items now carry deterministic `target_href`, while validation-attention rows additionally expose `Create Annotation Draft` as a direct action link into `annotations.html` with the full replay-attention prefill context preserved; `_render_readiness(...)` now renders safe-gated `analyst-briefing-target-link` / `analyst-briefing-action-link` anchors in the primary-focus summary and the item table; targeted RED/GREEN tests were added in `tests/unit/test_local_gui.py`, focused GUI regression passed (`23 passed in 108.20s`), full suite `534 passed in 345.76s`, and GUI evidence is visible in `build/local_gui/_val_wp014_briefing_links/readiness.html`; commit `ad7dc96`
- VAL-WP-015 release-demo-package contextual handoff links: the management-/reviewer-facing release package now preserves contextual analyst-briefing navigation instead of collapsing back to static page names; `src/siasa/readmodels/release_demo_package.py` now carries `primary_item_target_href`, `primary_item_action_href`, and row-level `target_href` / `action_href`, uses contextual hrefs for guided step `C3-02`, and renders safe-gated `analyst-briefing-target-link` anchors (plus action links when present) in `release_package.html`; `tests/unit/test_local_gui.py` now asserts `release_demo_package.json` href persistence and `release_package.html` link rendering; targeted regression `2 passed in 2.43s`, focused GUI regression `23 passed in 104.73s`, full suite `534 passed in 339.32s`; GUI evidence `build/local_gui/_val_wp015_release_handoff_links_demo/release_package.html`; commit `e5d3d05`
- VAL-WP-016 reviewer-flow contextual fallback hardening: fallback-built release/demo walkthroughs no longer degrade validation review steps back to generic `validation.html`; `src/siasa/readmodels/release_demo_package.py` now emits contextual `target_href`s for fallback-generated `country_gap`, `validation_attention`, `traceability_risk`, `operability_cluster`, and `stale_priority` items, synthesizes `validation_focus_href` from the active validation view model when no replay-attention watchlist item is present, and keeps guided step `C3-04` focused on the exact validation slice; `tests/unit/test_local_gui.py` now asserts fallback-path `review_sequence` and exported `release_demo_package.json` contextual href persistence; targeted regression `2 passed in 2.43s`, focused GUI regression `23 passed in 100.65s`, full suite `534 passed in 332.78s`; GUI evidence `build/local_gui/_val_wp016_reviewer_flow_context/release_demo_package.json`; commit `80c7a58`
- VAL-WP-017 stakeholder cover-sheet contextual start-here alignment: the stakeholder-facing cover sheet no longer restarts the reviewer at generic `readiness.html` when a richer contextual primary focus already exists; `src/siasa/readmodels/release_demo_package.py` now derives `stakeholder_cover_sheet.start_here` from the current primary item (`target_href` / `target_page` / `recommended_next_check`) so page, label, href, and reason stay aligned with the same focused Coverage/Validation slice already used in the reviewer handoff; `tests/unit/test_local_gui.py` now asserts exported `release_demo_package.json` points `start_here` to the contextual country-gap href in the fallback fixture path; targeted regression `2 passed in 2.64s`, focused GUI regression `23 passed in 106.06s`, full suite `534 passed in 336.64s`; GUI evidence `build/local_gui/_val_wp017_cover_sheet_start_here_context/release_package.html`; commit `commit pending`

After that, proceed with the next highest-value validation depth or analyst interpretation package.
