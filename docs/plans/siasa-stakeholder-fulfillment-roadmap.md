...
- AP-18 is now closed (operator steering surfaced in readiness UX); therefore the next serial planning focus is AP-19: add release-failure-drill trend baselining so recurring no-go clusters can be tracked over time (worsening/stable/improving) rather than treated as isolated snapshots
