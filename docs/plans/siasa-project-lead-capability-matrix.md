...
| --- | --- | --- | --- | --- |
| P2 live-runtime breadth extension: first core-focus-expanded subset | Extend governed live runtime breadth beyond `core-focus-initial` by adding `IND` as a seventh country in a reusable named subset while preserving honest `TWN` A/B scoping and making the existing `POL/B` transient-gap retry logic reusable for this broader subset | Done | Runtime now supports `--pilot-set core-focus-expanded`, targeted tests and full suite pass, `RUN-LIVE-CORE-EXP-003` completed successfully with 7 updated countries, and live evidence still honestly shows the remaining `POL/B` release-gap pattern rather than hiding it | Broadens governed exploratory breadth by one additional P1 country (`IND`) while keeping the repo/runtime honest about remaining release blockers in the broader subset |
| P2 live-runtime breadth extension: initial core-focus subset | Expand governed live runtime breadth beyond the 4-country representative pilot by adding a first broader named subset with verified live-source coverage for `RUS` and `CHN` while preserving honest `TWN` A/B scoping | Done | Runtime now supports `--pilot-set core-focus-initial`, targeted tests and full suite pass, CLI preserves named pilot-set semantics, and live probe `RUN-LIVE-CORE-INIT-001` succeeded with 6 updated countries and `release_verdict=ready` | Extends governed breadth from the original representative quartet to a first broader reusable subset without overclaiming full MVP-country support |
