...
| Representative multi-country runtime breadth | More than one governed country materially supported in latest outputs | Partial | Multi-country runtime/artifact path works; current repo evidence now includes consecutive successful representative 4-country live probes with `SRC-GDELT-DOC` recovering under the hardened runtime path, and the GDELT events window now aggregates multiple recent exports so POL/TWN no longer collapse out of Domain B | Breadth is now materially stronger, but TWN still lacks a governed Domain D live source in the current pilot set, so stakeholder breadth is not yet equivalent to balanced full-domain completeness | Runtime code under `src/siasa/runs/live_runtime.py`, probe artifacts under `build/run_artifacts/_resil_probe_1` / `_resil_probe_2` / `build/run_artifacts/_coverage_probe_1`, and readmodels such as `world_map.json` | Full suite pass, runtime-related tests, and representative live probe evidence | Recent P2 runtime hardening and coverage packages | P1/P2 reassessment after TWN Domain D support decision |
...
| P2 data-complete country coverage MVP | Move from narrow proof to representative multi-country runtime coverage | Partial | Current repo evidence now includes a representative live probe where all four countries reach Domain B coverage after widening the GDELT events window, and TWN Domain D is now explicitly surfaced as `not_configured_for_runtime` instead of a misleading scope leak from World Bank | `Representative multi-country runtime breadth` remains Partial until the project decides whether TWN Domain D gets a real governed live source or is intentionally excluded from that domain |
...
- next work package: explicit TWN Domain D support decision and closure path

Why this is next:
- representative 4-country live runtime is now materially more complete: the widened GDELT events window removes the earlier POL/TWN Domain B collapse, and a fresh live probe shows all four countries at `S1`
- the dominant remaining gap is now narrower and more architectural: TWN still has no governed Domain D live source in the pilot set, and the runtime now surfaces that honestly as `not_configured_for_runtime`
- this means the next highest-value step is no longer generic transport hardening or broad coverage triage, but deciding whether to (a) integrate a real Domain D source for TWN, or (b) formally scope TWN out of Domain D for the governed pilot with corresponding roadmap/matrix alignment
