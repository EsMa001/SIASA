...
| Real source access | Productive use of real external sources | Partial | Source-access assessment and governed live runtime pilot exist; real adapter baselines exist for selected sources; governed named pilot sets now cover the stable 4-country representative subset, the broader 6-country `core-focus-initial` subset (`UKR,RUS,CHN,TWN,ISR,POL`), the 7-country `core-focus-expanded` subset (`UKR,RUS,CHN,TWN,ISR,IND,POL`), and the new 9-country `core-focus-broader` subset (`UKR,RUS,CHN,TWN,IRN,ISR,TUR,IND,POL`) | Full production-grade breadth and stable access across intended source classes remains incomplete, but the broader subset now also has a successful `release_verdict=ready` probe with `IRN` and `TUR` included; the remaining obvious breadth candidates are now `PAK` and `GEO` rather than the earlier IRN/TUR gap | Current roadmap status in `docs/plans/siasa-stakeholder-fulfillment-roadmap.md`, live runtime code under `src/siasa/runs/live_runtime.py`, and probe artifact bundles `build/run_artifacts/_core_focus_initial_probe_1`, `build/run_artifacts/_core_focus_expanded_probe_4`, and `build/run_artifacts/_core_focus_broader_probe_1` | Repo tests, full-suite pass, and live probe evidence for `core-focus-initial` / `core-focus-expanded` / `core-focus-broader` | P0 readiness and later breadth packages advanced | Continue governed breadth expansion or deepen validation/reference cases |
| Representative multi-country runtime breadth | More than one governed country materially supported in latest outputs | Done | Multi-country runtime/artifact path works; representative live probes show the stable 4-country governed subset, the runtime also supports `core-focus-initial` with `UKR,RUS,CHN,TWN,ISR,POL`, `core-focus-expanded` with added `IND`, and `core-focus-broader` with added `IRN` and `TUR`; TWN remains explicitly scoped to A/B while the added `RUS`, `CHN`, `IND`, `IRN`, and `TUR` countries run with A/B/D | Full-domain breadth beyond these subsets remains a later expansion topic, but the 9-country `core-focus-broader` subset now has a successful ready-state probe rather than only narrower exploratory breadth evidence | Runtime code under `src/siasa/runs/live_runtime.py`, probe artifacts under `build/run_artifacts/_resil_probe_1` / `_resil_probe_2` / `_coverage_probe_1` / `_scope_probe_1` / `build/run_artifacts/_core_focus_initial_probe_1` / `build/run_artifacts/_core_focus_expanded_probe_4` / `build/run_artifacts/_core_focus_broader_probe_1`, and readmodels such as `_core_focus_broader_probe_1/readmodels/world_map.json` and `_core_focus_broader_probe_1/readmodels/readiness.json` | Full suite pass, runtime-related tests, and successful `RUN-LIVE-CORE-INIT-001` / `RUN-LIVE-CORE-EXP-RETRY-004` / `RUN-LIVE-CORE-BROAD-001` live probe evidence | Current P2 breadth extension package | Next breadth slice to remaining MVP countries (`PAK`, `GEO`) or validation/reference-case depth |
...
- next work package: either extend governed live-source/runtime breadth from `core-focus-broader` to the remaining obvious P1 candidates (`PAK`, `GEO`), or expand the validation/reference-case library if stakeholder value is judged higher
