...
| Real source access | Productive use of real external sources | Partial | Source-access assessment and governed live runtime pilot exist; real adapter baselines exist for selected sources; governed named pilot sets now cover the stable 4-country representative subset, the broader 6-country `core-focus-initial` subset (`UKR,RUS,CHN,TWN,ISR,POL`), and an additional 7-country `core-focus-expanded` subset (`UKR,RUS,CHN,TWN,ISR,IND,POL`) for broader exploratory breadth | Full production-grade breadth and stable access across intended source classes remains incomplete, but the broader subset now also has a successful `release_verdict=ready` probe after extra GDELT DOC recovery budget rather than being stuck behind the earlier residual `POL/B` gap pattern | Current roadmap status in `docs/plans/siasa-stakeholder-fulfillment-roadmap.md`, live runtime code under `src/siasa/runs/live_runtime.py`, and probe artifact bundles `build/run_artifacts/_core_focus_initial_probe_1`, `build/run_artifacts/_core_focus_expanded_probe_3`, and `build/run_artifacts/_core_focus_expanded_probe_4` | Repo tests, full-suite pass, and live probe evidence for `core-focus-initial` / `core-focus-expanded` | P0 readiness and later breadth packages advanced | Continue governed breadth expansion or deepen validation/reference cases |
| Representative multi-country runtime breadth | More than one governed country materially supported in latest outputs | Done | Multi-country runtime/artifact path works; representative live probes show the stable 4-country governed subset, the runtime also supports `core-focus-initial` with `UKR,RUS,CHN,TWN,ISR,POL`, and a broader `core-focus-expanded` subset now adds `IND`; TWN remains explicitly scoped to A/B while the added `RUS`, `CHN`, and `IND` countries run with A/B/D | Full-domain breadth beyond these subsets remains a later expansion topic, but the 7-country `core-focus-expanded` subset now also has a successful ready-state probe instead of only exploratory breadth evidence | Runtime code under `src/siasa/runs/live_runtime.py`, probe artifacts under `build/run_artifacts/_resil_probe_1` / `_resil_probe_2` / `_coverage_probe_1` / `_scope_probe_1` / `build/run_artifacts/_core_focus_initial_probe_1` / `build/run_artifacts/_core_focus_expanded_probe_3` / `build/run_artifacts/_core_focus_expanded_probe_4`, and readmodels such as `_core_focus_expanded_probe_4/readmodels/world_map.json` and `_core_focus_expanded_probe_4/readmodels/readiness.json` | Full suite pass, runtime-related tests, and successful `RUN-LIVE-CORE-INIT-001` / `RUN-LIVE-CORE-EXP-003` / `RUN-LIVE-CORE-EXP-RETRY-004` live probe evidence | Current P2 breadth extension package | Next breadth slice to additional MVP countries or validation/reference-case depth |
...
| P2 live-runtime hardening: broader GDELT DOC recovery budget | Allow a broader governed live subset to recover from repeated isolated `SRC-GDELT-DOC` partial-success attempts instead of giving up after a single rerun | Done | `run_governed_live_pipeline()` now uses an extra default retry for 7-country subsets, targeted and full runtime tests pass, full suite passes, and live probe `RUN-LIVE-CORE-EXP-RETRY-004` completed with `release_verdict=ready` | Improves practical stability of the broader 7-country governed subset without broadening retry behavior indiscriminately for smaller subsets |
...
- next work package: broaden governed live-source/runtime breadth beyond the current representative pilot, or expand the validation/reference-case library if stakeholder value is judged higher
