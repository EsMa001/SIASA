...
- next work package: broader live-source/runtime hardening

Why this is next:
- the readiness-honesty slice is now closed: artifact presence/absence is explicit and `validation_backtest` is visibly present in the current latest bundle
- release readiness is still blocked by real runtime/source failures, especially fetch instability that currently surfaces as `failed_source:SRC-GDELT-DOC`
- this means the biggest remaining delta is operational robustness, after which representative breadth expansion can be prioritized with clearer evidence
