...
| Real source access | Productive use of real external sources | Partial | Source-access assessment and governed live runtime pilot exist; real adapter baselines exist for selected sources; governed named pilot sets now cover both the 4-country representative subset and a first broader 6-country `core-focus-initial` subset (`UKR,RUS,CHN,TWN,ISR,POL`) | Full production-grade breadth and stable access across intended source classes remains incomplete | Current roadmap status in `docs/plans/siasa-stakeholder-fulfillment-roadmap.md`, live runtime code under `src/siasa/runs/live_runtime.py`, and probe artifact bundle `build/run_artifacts/_core_focus_initial_probe_1` | Repo tests, full-suite pass, and live probe evidence for `core-focus-initial` | P0 readiness and later breadth packages advanced | Continue governed breadth expansion or deepen validation/reference cases |
| Representative multi-country runtime breadth | More than one governed country materially supported in latest outputs | Done | Multi-country runtime/artifact path works; representative live probes show the stable 4-country governed subset, and the runtime now also supports a broader named `core-focus-initial` live subset with `UKR,RUS,CHN,TWN,ISR,POL`; TWN remains explicitly scoped to A/B while the added `RUS` and `CHN` countries run with A/B/D | Full-domain breadth beyond this initial broader subset remains a later expansion topic, but the governed runtime is now no longer limited to the original representative quartet | Runtime code under `src/siasa/runs/live_runtime.py`, probe artifacts under `build/run_artifacts/_resil_probe_1` / `_resil_probe_2` / `_coverage_probe_1` / `_scope_probe_1` / `build/run_artifacts/_core_focus_initial_probe_1`, and readmodels such as `_core_focus_initial_probe_1/readmodels/world_map.json` | Full suite pass, runtime-related tests, and successful `RUN-LIVE-CORE-INIT-001` live probe evidence | Current P2 breadth extension package | Next breadth slice to additional MVP countries or validation/reference-case depth |
