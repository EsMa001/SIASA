...
- next work package: `P1` closure package for latest-bundle validation completeness / readiness honesty

Why this is next:
- the recent analyst-depth GUI gaps (annotation workflow, baseline/historical comparison overlays, and source-dependency/source-origin groundwork) are now all closed at the current static-baseline level
- the most important remaining project-lead gap is therefore no longer another GUI slice, but runtime honesty: the latest governed bundle should not remain weaker than the visible product baseline
- after that, the next decision should be between broader runtime hardening and representative country-breadth expansion, guided by the capability matrix
