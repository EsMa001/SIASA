...
| Validation / backtest review | Compare expected vs observed outcomes with governed evidence | Partial | Validation page exists, latest governed bundle now carries both `validation_backtest` and persisted `readiness.json`, and readiness exposes explicit artifact presence/absence status | The validation view is still a runtime-support check rather than a broader historical reference-case library, so analytical depth can still improve later | `validation.html`, `readiness.html`, `build/run_artifacts/latest/readmodels/readiness.json`, `build/run_artifacts/latest/readmodels/validation_backtest.json`, `readmodels/system_status.json` | Validation-related tests in repo; full suite passing; browser/runtime verification | `P1` closure slice | Broader live-source/runtime expansion or richer validation library |
