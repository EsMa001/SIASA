...
| Validation / backtest review | Compare expected vs observed outcomes with governed evidence | Partial | Validation page exists, latest governed bundle now carries both `validation_backtest` and persisted `readiness.json`, readiness exposes explicit artifact presence/absence status, and the validation artifact/view now includes a multi-case portfolio summary across all governed countries in the run | The validation view is still based on runtime-support cases rather than a richer historical reference-case/backtest library, so analytical depth can still improve later | `validation.html`, `readiness.html`, `build/run_artifacts/latest/readmodels/readiness.json`, `build/run_artifacts/latest/readmodels/validation_backtest.json`, `readmodels/system_status.json` | Validation-related tests in repo; full suite passing; browser/runtime verification | `P1` closure slice | Broader live-source/runtime expansion or richer historical validation library |
...
- next work package: choose between expanding governed breadth beyond the now-complete core-focus subset into broader MVP/P2 countries, or deepening the validation/reference-case library if stakeholder value is judged higher
...
- the remaining highest-value deltas are therefore no longer about whether the current pilot can explain its own artifact state, but about how far that governed live runtime and evidence library should expand next
- the immediate choice should now be made between breadth and depth:
  - breadth = extend live-source/runtime support beyond the current representative governed subset
  - depth = turn the current runtime-support validation view into a richer reference-case / backtest library
