...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum required combined C/E utilization ratio [0..1]",
    )
    parser.add_argument(
        "--allowed-verdicts",
        type=str,
        default="green,amber",
        help="Comma-separated allowed governance verdicts (e.g., green or green,amber)",
    )
    parser.add_argument(
        "--max-failed-sources",
        type=int,
        default=3,
        help="Maximum allowed failed source count",
    )
...
def main() -> int:
    ...
    policy = LiveProbePolicy(
        min_combined_ce_ratio=args.min_combined_ce_ratio,
        allowed_verdicts=allowed_verdicts or ("green", "amber"),
